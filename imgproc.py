...
def image_resize(image: Any, scale_factor_list: list[float, float], antialiasing: bool = True) -> Any:
    ...
    Args:
        ...
        scale_factor_list (list[float, float]): Scale factor list. The same scale applies for both height and width.
        antialiasing (bool): Whether to apply antialiasing when down-sampling operations.
...
def center_crop(image: np.ndarray, image_size: list[int, int]) -> np.ndarray:
    ...
    Args:
        ...
        image_size (list[int, int]): The size of the captured image area.
...
def random_crop_torch(
    ...
    Args:
        gt_images (Tensor): High resolution images
        patch_gt_image_size (list[int, int]): The size of the captured high-resolution image area. (H, W)

    Returns:
        patch_gt_images (Tensor): Small gt patch images
