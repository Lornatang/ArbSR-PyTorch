...
def main():
    ...
    arbsr_model, ema_arbsr_model = build_model()
    print(f"Build `{config.model_arch_name}` model successfully.")

    criterion = define_loss()
    print("Define all loss functions successfully.")

    optimizer = define_optimizer(arbsr_model)
    ...
    if config.pretrained_model_weights_path:
        arbsr_model = load_state_dict(arbsr_model, config.pretrained_model_weights_path)
    ...
    for epoch in range(start_epoch, config.epochs):
        train(arbsr_model,
              ...
              scaler,
              writer)
        psnr, ssim = validate(arbsr_model, test_prefetcher, epoch, writer, psnr_model, ssim_model, "Test")
        print("\n")
        ...
        save_checkpoint({"epoch": epoch + 1,
                         "best_psnr": best_psnr,
                         "best_ssim": best_ssim,
                         "state_dict": arbsr_model.state_dict(),
                         "ema_state_dict": ema_arbsr_model.state_dict(),
...
def define_optimizer(arbsr_model) -> optim.Adam:
    optimizer = optim.Adam(arbsr_model.parameters(),
                           config.model_lr,
                           config.model_betas,
                           config.model_eps,
...
def train(
        arbsr_model: nn.Module,
        ema_arbsr_model: nn.Module,
    ...
    while batch_data is not None:
        ...
        # Crop
        width_upscale_factor = random.choice(config.upscale_factor_list)
        height_upscale_factor = random.choice(config.upscale_factor_list)

        gt = imgproc.random_crop_torch(gt,
                                       [round(config.lr_image_size * width_upscale_factor),
                                        round(config.lr_image_size * height_upscale_factor)])

        # Resize
        lr = F_torch.interpolate(gt, size=(config.lr_image_size, config.lr_image_size), mode="bicubic")

        # Clamp and round
        lr = torch.clamp((lr * 255.0).round(), 0, 255) / 255.

        # Initialize generator gradients
        arbsr_model.zero_grad(set_to_none=True)

        # Mixed precision training
        with amp.autocast():
            sr = arbsr_model(lr, width_upscale_factor, height_upscale_factor)
        ...
        # Update EMA
        ema_arbsr_model.update_parameters(arbsr_model)
...
def validate(
    ...
    with torch.no_grad():
        while batch_data is not None:
            ...
            sr = arbsr_model(lr, config.upscale_factor, config.upscale_factor)
